...
def _article_body_len(url: str) -> int:
    try:
    ...
    for line in text.splitlines():
        line = line.strip()
        if line and line not in {"Go back", "Back"} and "akorda.kz" not in line.lower():
            lines.append(line)
...
def _rows_from_page(lang: str, page: int) -> list[dict[str, str]]:
    ...
    rows = []
    pattern = re.compile(
        r'(?is)<h3[^>]*>\s*<a[^>]+href=["\']([^"\']+)["\'][^>]*>(.*?)</a>\s*</h3>(.{0,700}?)(?:<h5[^>]*>(.*?)</h5>)?'
    )
    for match in pattern.finditer(html):
        ...
        day = _parse_day(match.group(4) or match.group(3))
        rows.append({
